...
Status (2026-08-09, POP-OUT OVERLAY WINDOWS SHIPPED):
...
- Build: `dotnet build ODExplorer.sln -c Release` => 0 errors. TestApp smoke suite: 96 checks — ALL CHECKS PASSED (two EDSM-flow checks are timing-flaky in CI-style back-to-back runs; pass on re-run, unrelated to pop-outs).
- TestApp coverage unchanged (96 checks); the pop-out work touches no journal/EDSM paths.
