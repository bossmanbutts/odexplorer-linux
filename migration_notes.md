...
Status (2026-08-09, TOAST NOTIFICATIONS + STARTUP RESUME SHIPPED):
...
- Triggers wired into the compiled `Stubs/ExplorationDataStoreImpl.cs`: worth-mapping (`TriggerBodyEventIfLive`), new-bio-scanned (`TriggerBioUpdatedIfLive`, suppressed for on-foot codex dupes), and notable-body checks on `FSSBodySignals`. `SystemBody` gained `Eccentricity`/`SemiMajorAxis`/`TerraformState` (mapped from Scan events). Still not wired: valuable-exo-body, new-codex, new-species toasts (upstream fired these from the bio-prediction path).
...
- TestApp smoke test now 56 checks (7 toast-emission + disabled-suppression, 2 resume checks) — ALL CHECKS PASSED. Build: `dotnet build ODExplorer.sln -c Release` => 0 errors.
- Remaining for full parity: wire valuable-exo/new-codex/new-species toasts, first-run onboarding, real EDSM system-details flow (`OnSystemUpdatedFromEDSM` declared but never raised), CartographicView layout polish, real ODUtils/EliteJournalReader package swap, and in-app persistence of Display/Settings via the settings DTO path.
