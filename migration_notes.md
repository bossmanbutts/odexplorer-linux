...
Status (2026-08-09, TOAST NOTIFICATIONS + STARTUP RESUME + SETTINGS PERSISTENCE + BIO-DISCOVERY TOASTS + EDSM SYSTEM-DETAILS FLOW SHIPPED):
...
- TestApp smoke test now 68 checks (7 toast-emission + disabled-suppression, 2 resume checks, 5 settings round-trip, 3 live bio-discovery toasts, 6 EDSM-flow checks) — ALL CHECKS PASSED. Build: `dotnet build ODExplorer.sln -c Release` => 0 errors.
...
- Remaining for full parity: first-run onboarding, CartographicView layout polish (Detailed/Classic/Horizontal/Extended), real ODUtils/EliteJournalReader package swap.
