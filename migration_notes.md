...
Status (2026-08-11, REAL ODUtils EXO ENGINE COMMITTED — `ea50891`):
- **The real engine was never committed**: `git status` showed `?? src/ODExplorer.Core/Exobiology/` (the full engine) alongside ` D` for the BioScan stubs — so the previous "complete exo prediction pipeline" commit built the stubbed `ExoPredictionEngine` while the real code sat untracked. Committed the whole engine: all 90 species (gravity/temperature/planet-class/atmosphere/region/volcanism/pressure rules), the material/star-gated variant matcher (`SpeciesBase.GetPredictions`), `ExoData` reflection init, `OrganicValues` pricing, `ExoPlanetBuilder` journal→`ExoPlanet` translation, and the 27k-line `RegionMap`. Deleted `ExoGalaxyData`/`ExoGalaxyPosition`/`ExoPredictionData`/`ExoPredictionEngine`/`Exobiology`/`GalacticRegionsMap`/`OrganicStubs` stubs.
- **TestApp rewritten** to run the real engine (`exo.GetPredictions(ExoPlanetBuilder.Build(...))`): gravity-window (Tectonicas ≤ 0.607g), thin vs thick CO2, Sagittarius-Carina region gate (Cactoida Lapis), nebula-distance gate (Electricae Radialem), material-gated variant (cadmium → Concha Renibus), Likely vs parent-star Unlikely variants, ≥1.05% SO2 (Recepta Umbrux), geysers + thin atmosphere (Bacterium Tela). Expected lists captured from engine output.
- **Bacterium Tela fix**: its volcanism branch tested the atmosphere description for `"Thin"` but the vendored descriptions are lowercase (`"thin carbon dioxide"`), making the branch dead code — now compared case-insensitively.
- **Hygiene**: deleted stray UI publish artifacts at the repo root (`ODExplorer.exe`/`.pdb`, `ODUtils.pdb`) and gitignored them; working tree is clean.
- **Verified**: `dotnet build ODExplorer.sln` 0 errors; TestApp ALL CHECKS PASSED (the occasional "1 CHECK FAILED" is a pre-existing timing flake in the live-toast polling test — passes on rerun).
- Remaining big-picture unchanged: (1) real-desktop end-to-end validation (manual); (2) ODUtils reconciliation of the non-exo stubs via the VM copy — the engine itself is now real and committed.

Status (2026-08-11, TODO LIST COMPLETION SESSION — CI FIX, ScanOrganic TOLERANCE, CLEANUP):
- **CI was silently broken**: `.github/workflows/ci.yml` pinned the .NET **8.0.x** SDK, but the Avalonia 12.1.0 analyzers need Roslyn ≥ 4.14, so the UI project fails with `CS0103: InitializeComponent does not exist` on the next push (verified by forcing `global.json` → 8.0.423). Bumped CI to `10.0.x`. Local default SDK is 10.0.302; `.NET 8` still fails the UI build — the migration-notes warning stands.
- **ScanOrganic must-map events silently dropped**: the vendored ELJR `ScanOrganicEvent.ScanType` used `StringEnumConverter` over `OrganicScanStage`, which *throws* on `"ScanType":"Organic"` (an older/valid journal value; this folder only ever emits Log/Sample/Analyse). `JournalEventMapper.Map` catches and returns null, so a genuine scan was silently lost (confirmed via the JournalReplay harness — the line landed in the "unmapped by event" bucket). Switched the property to `ExtendedStringEnumConverter<OrganicScanStage>` (falls back to default instead of throwing). Now both `"Organic"` and `"Log"` ScanType lines map. Re-verified against the user's real journal folder: 1798/1798 mapped, ALL REPLAY CHECKS PASSED.
- **Stub audit re-pass**: only `GalacticRegions` in `ODExplorer.Stubs` (`ODUtilsModels.cs`) has callers; the duplicate `StarSystem`/`SystemBody`/`Parent`/`Ring`/`Owner` types were dead (the pipeline uses `ODUtils.Models.*`) and are deleted. Confirmed the only remaining `=> null`/`=> true` no-ops (`GetNavRoute`, `Odyssey`, `GetGoverningStar`, `CanExecute`) have no callers or are normal command behavior.
- **Regression check added**: TestApp now asserts a journal `Scan` with `"Atmosphere":"carbon dioxide"` yields `body.Atmosphere == CarbonDioxide` (the specific failure mode of the previous session's fix).
- **Verified**: TestApp smoke suite ALL CHECKS PASSED (incl. new atmosphere check), NUnit 27/27, `dotnet build ODExplorer.sln -c Release` 0 errors, JournalReplay green on synthetic (9-line full-event journal) and real (1798-line) inputs.
- Remaining big-picture unchanged: (1) real-desktop end-to-end validation (manual); (2) ODUtils reconciliation via the VM copy.

Status (2026-08-11, JOURNAL ATMOSPHERE PARSE FIXED — PREDICTION NOW WORKS END-TO-END):
- **Root cause of the "predictions never fill" bug**: the vendored `EliteJournalReader` `AtmosphereDescription` enum (JournalDescriptions.cs) had `[Description("Carbon Dioxide Atmosphere")]`-style strings, but the journal emits `"Atmosphere": "carbon dioxide"` (no "Atmosphere" suffix). `ExtendedStringEnumConverter.ToEnum` couldn't map it, so every journal-parsed body got `Atmosphere = None` (diag showed `atm=None`) and no atmospheric prediction rule could ever match — `Predict()` returned 0 species for real Scan data even though the direct `TestApp`-constructed bodies (which set `AtmosphereClass.CarbonDioxide` by hand) predicted fine.
- **Fix**: rewrote all ~160 `[Description]` attributes on `AtmosphereDescription` to the real journal strings ("carbon dioxide", "carbon dioxide-rich", "hot thick carbon dioxide-rich", "No atmosphere", ...). The enum member names are unchanged (snake_case), so `JournalEventMapper.GetAtmosphereClass` still matches on `ToString()` tokens exactly as before. `StringValue()`/descriptions are unused elsewhere in this repo (verified), so the change is display-neutral.
- **Store**: `ExplorationDataStoreImpl` already mapped `body.Atmosphere = GetAtmosphereClass(scanEvt.Atmosphere)` correctly — it was the enum parse feeding it that was broken. No store change needed.
- **Verified**: the new prediction-pipeline smoke checks ("history prediction fills scanned-body placeholders", "go-live catch-up tracks FSS-only bodies", "live prediction raises valuable exo body toast") pass; `[diag] atm=CarbonDioxide`, `direct predict` returns the 6 expected species; ALL CHECKS PASSED; 27/27 NUnit; `ODExplorer.Core` + `EliteJournalReader` build 0 errors.
- Remaining big-picture unchanged: (1) real-desktop end-to-end validation; (2) ODUtils reconciliation via the VM copy.

Status (2026-08-11, GALAXY-POSITION PREDICTION + PREDICTION PIPELINE COMPLETED):
- **Galaxy-position prediction wired** (commits `39024a9` + this session): new `Stubs/ExoGalaxyPosition.cs` — BioScan-compatible RLE region-grid lookup (`FindRegionId` over `ExoGalaxyData.RegionRows`), region membership/negation (`MatchesRegions`, `!name` elimination), and nebula matching (`MatchesNebula`: sector-name prefix + nearest large/planetary nebula distance; skipped when x is unknown). `ExoGalaxyData.cs` is now consumed (was "unused" per prior notes). `ExoPredictionEngine` evaluates `Regions`/`Nebula` from `system.Position`/`system.Name`.
- **ExoPredictionData regenerated**: the region/nebula-gated rules that the committed data had dropped entirely (whole rules removed for species like Fungoida Stabitis, Osseus Fractus/Pellebantus, Fungoida Gelata, Stratum Excutitus/Limaxus/Cucumisis/Frigus, Tubus Conifer, Aleoida Spica, Cactoida Lapis, Electricae Radialem) are back. Generator is reproducible (re-run → byte-identical file).
- **Cross-validated**: expected species lists produced by a Python mirror of the engine run over the generated rule data (see `/tmp/opencode/bioscan/verify_engine.py`); the C# engine matches exactly. TestApp gained region-gate checks (Cactoida Lapis present at Sol / absent in Elysian Shore; Tussock Cultro↔Divisa swap) and nebula-gate checks (Electricae Radialem present in Pleiades Sector / absent far from nebulae). ALL 127 CHECK(S) PASSED; `dotnet build ODExplorer.sln -c Release` => 0 errors.
- **Prediction pipeline completed**: (a) go-live re-prediction — `ParserStore_OnParserStoreLive` now re-predicts every bio body still holding "Not Predicted" placeholders and catches FSS-only bodies (signals found but never individually scanned during history) into `OrganicScanItems`; (b) placeholder expansion — `UpdateBioPredictions`/`UpdateSignalsFound` now top placeholders up to `BiologicalSignals` (was: placeholders only created when the list was empty, and extra signals were ignored); (c) prediction-time toasts — filling a prediction while live fires the `ValuableBioPlanet` toast via `NotifyBioDiscoveries` (NewBioSpecies/NewBioCodexEntry stay scan/codex-only; go-live history catch-up suppresses toasts to avoid startup spam).
- **Remaining big-picture**: (1) real-desktop end-to-end validation (audio/notifications/clipboard/tray/pop-outs — headless env can't launch the UI); (2) ODUtils reconciliation — in progress, user has ODExplorer installed on a VM to recover the real ODUtils surface.
...
- #3 Stub-accuracy pass: audited every file under `src/ODExplorer.Core/Stubs/` for genuine no-op/mis-mapped logic instead of approximation-level (documented) behavior. Two real bugs found and fixed:
  ...
  - `ODUtilsModelsBridge.cs` `ShipMaterials.Name_AsMaterial => PlanetMaterial.None` never mapped scan `Materials` names to jumponium flags. Now a `ToLowerInvariant()` switch over carbon/vanadium/germanium/cadmium/niobium/yttrium/polonium (anything else → `None`), fed by `ExplorationDataStoreImpl` line ~907 (`m.Name.ToString()` from the ELJR Scan event).
  - Everything else checked out: `Exobiology.cs` species table is game-correct (93 species; values and colony ranges verified against the codex — Stratum Tectonicas 1.6M, Electricae Radialem 1.2M, Fungoida Setisis 1.0M, Bacterium Acerosis 100K/colony range 2; only region/variant-palette fields are approximation-level). `JournalPipelineTypes.cs` is pure protocol types + event-name mapping (unknown → `Other`). `JournalEventMapper.cs` delegates parsing to real ELJR and its value helpers (star/planet/atmosphere/volcanism/region/FSS value) are documented approximations. `GetNavRoute()`, `Odyssey => true`, and the instance `SystemBody.GetGoverningStar()` have zero callers (harmless, kept to mirror the ODUtils surface).
  - TestApp smoke suite grew 96 → 116 checks: material name→flag mapping (incl. uppercase), non-jumponium → `None`, the full `ContainsAllShipMaterials` matrix, three `StarSystemViewModel.GreenSystem` scenarios (Premium/Standard/None), and 10 exo-table checks (species count, values, colony range, unknown-codex null, species/variant names, region availability). ALL 116 PASS; `dotnet build ODExplorer.sln -c Release` => 0 errors.
- #4 Real-journal validation harness: new `src/JournalReplay/` console project (`JournalReplay --dir <journal-folder> [--db <sqlite-file>]`). Headless replay of a live game's journal directory through the exact production pipeline (EF Sqlite DB → `JournalParserStore` → `OrganicCheckListDataStore` + `ExplorationDataStore`). It (a) pre-scans every line through `JournalEventMapper` and buckets mapped/unmapped by event name, failing if any store-relevant ("must-map") event is left unmapped; (b) runs `ReadNewDirectory` until live; (c) asserts state sanity (commander found, current system from the last real FSDJump, organic bodies recorded, scanned species in the checklist, DB journal-entry count == mapped-line count for persistence parity). Network-free via an offline `EdsmApiService` override; NLog configured so store-swallowed exceptions print to console. Verified against the user's real journal folder (`/mnt/twins/SteamLibrary/.../Saved Games/Frontier Developments/Elite Dangerous`, 15 files, 1798 lines): ALL REPLAY CHECKS PASSED — 1798/1798 lines mapped (0 unmapped), every pipeline event parsed (143 Scan, 22 FSDJump, 12 FSSBodySignals, 4 ScanOrganic, 2 CodexEntry, 1 SellOrganicData), current system `Col 285 Sector RN-T d3-60` (Bubble), 4 organic bodies recorded, 1 unique species scanned + 1 sold, 1798 DB entries (full parity).
- Not committed (pending review): the two stub fixes + TestApp checks + the new JournalReplay project.
- Run the harness yourself: `dotnet run --project src/JournalReplay/JournalReplay.csproj -c Release -- --dir "<your Saved Games>/Frontier Developments/Elite Dangerous"`.

Prior status (2026-08-09, POP-OUT OVERLAY WINDOWS SHIPPED + DEAD STORE COPIES REMOVED):
- The VB/EX overlay buttons (Carto footer) and the startup-restore path were silent no-ops: `MainViewModel.OpenPopoutRequested` had no subscriber anywhere in the UI project.
- New `UI.Avalonia/Views/PopOutWindow.cs` (code-only `Window`): consumes the core `PopOutBase` overlay (`Title`, saved `Position` geometry incl. `State`, `AlwaysOnTop`, `ShowInTaskBar`, `PopOutMode` → `Opacity` 1.0/0.65/0.2, `ShowTitle` → empty title bar), hosts the overlay content bound to the `MainViewModel` data context, and mirrors geometry back into `popOut.Position` on move/resize/state-change. `Closing` calls `MainViewModel.OnPopOutClose(popOut)` (now public) so `PopOutParams` are persisted as inactive and the overlay leaves `ActivePopOuts`. Subscribed in `MainWindow.axaml.cs` (`OnOpenPopoutRequested` → `new PopOutWindow(vm, popOut).Show(this)`).
- `ModelsPopOutBaseStub.cs` `PopOutBase` gained `ForceCloseRequested`/`ResetRequested` events raised from `ForceClose()`/`InvokeReset()`; `PopOutWindow` closes on `ForceCloseRequested` (parser offline) WITHOUT persisting the closed state so the commander's Active restore flag survives, and re-applies geometry on `ResetRequested` (the "reset pop-out positions" path). User-initiated closes (X button) persist `Active=false` as before.
- New overlay views bound to `MainViewModel`: `Views/PopOutOverlays/SystemBodiesOverlayView.axaml` (DataGrid of `CurrentSystem.Bodies`: ID/Name/Class/Distance ls/Mapped/FSS/Scan) and `ExobiologyOverlayView.axaml` (Organic-signals cards — same template as the Carto DetailedExo layout). Restored pop-outs are re-created by `OpenActivePopOuts()` via AppDomain reflection over `PopOutBase` subtypes.
- Dead real-store copies deleted: `Stores/ExplorationDataStore.cs` (1303 lines), `Stores/OrganicCheckListDataStore.cs` (433), `Stores/JournalParserStore.cs` (194) — all were `Compile Remove`d and could not even compile (they call real-ODUtils types like `ExoPlanet`/`ExoData.GetPredictions`/`GetParentStars` that this repo's stubs don't provide). Verified the live `Stubs/*Impl.cs` implementations are behavioral supersets: same public/protected members, same journal-event cases (dead-only `NavRouteEvent` is unreachable — its handler needs `parserStore.GetNavRoute()`, stubbed to null), and the sole dead-only method `ForceParsePredictions` has zero callers. `ODExplorer.Core.csproj` no longer needs the `Stores\**` Remove/Include whitelist; the only remaining exclusion is the Windows-only `SystemBodyViewModelDistanceFromArrivalComparer`.
- Known limitation (pre-existing, unchanged by this cleanup): the genus→species exo-prediction engine (`ExoPlanet` + `ExoData.GetPredictions` + `GetExoBioPredictions` with prediction-time ValuableBioPlanet/NewSpecies/NewCodex toasts, DSS-history and go-live re-prediction) is NOT implemented in the live store — FSSBodySignals creates "Not Predicted" placeholders only. This shipped when the Stubs were built and the 96-check suite doesn't assert predicted species. Restoring it means reimplementing the ODUtils prediction model or re-adding the real ODUtils package; the deleted dead copy remains recoverable from git as a reference.
- Build: `dotnet build ODExplorer.sln -c Release` => 0 errors. TestApp smoke suite: 96 checks — ALL CHECKS PASSED (two EDSM-flow checks are timing-flaky in CI-style back-to-back runs; pass on re-run, unrelated to these changes).

Prior status (2026-08-09, UI STABILITY + RESPONSIVE LAYOUT):
- GUI-freeze fix (the "three toasts then the app hangs on first-run go-live" bug): `UI.Avalonia/Controls/ToastHost.cs` `Show()` ran `while (active.Count >= settings.MaxNotificationCount) RemoveOldest();` where `RemoveOldest` → `FadeOut` only removed the card after a 300ms `DispatcherTimer.RunOnce`, so `active.Count` never shrank inside the loop — once the active set was full, the UI thread spun forever. `RemoveOldest` now stops the timer and removes the card from `canvas`/`active` synchronously (loop terminates) and `MaxNotificationCount <= 0` is clamped to 1.
- Settings page didn't stretch with the window: the view root ScrollViewer used `HorizontalScrollBarVisibility="Auto"`, which measures content at infinite width, so the inner Grid's `*` column sized to its content instead of the viewport. Switched to `Disabled` so the star column fills the window (vertical scroll preserved; left column MinWidth 480 + right MinWidth 260 keep it from clipping at the 900px window minimum). Also dropped the fixed `Width="520"` on the Developer test-button bar (it wraps now) and raised the commander list `MaxHeight` 170 → 320 so it grows with window height.
- EdAstro POI overlap: the list-item row used `ColumnDefinitions="Auto,*,*,90"` — with two star columns Avalonia splits the space proportionally to each column's desired content width, so a long POI `Name` claimed nearly all of it and squeezed the location column into an unreadable sliver. Row is now `Auto,*,Auto,90` (Name is the only star and ellipsizes, location is `Auto` sized to its text), and the ListBox moved from an `Auto` outer column to `2*,3*` so items are always measured at a finite width.
- Commander-section overlap on stretched windows: list-item rows are now `Auto,*,Auto,Auto` with explicit `TextWrapping="NoWrap"` + a disabled ListBox horizontal scrollbar (JournalPath is the only star), and the JournalPath/LastFile `TextBox`es no longer use `Grid.ColumnSpan="2"`, so their text can't run underneath the Change-Logs-Folder / Reset-Last-File buttons.
- Verified no other instances of the two-star item-template pattern or the eviction-loop pattern remain in `Views/`/`Controls/`. TestApp smoke suite: 69 checks — ALL CHECKS PASSED. `dotnet build ODExplorer.sln -c Release` => 0 errors.
- Test coverage: added 25 direct checks for `NotificationStore.CheckForNotableNotifications` — every `BodyNotification` threshold (DiverseLife, SmallPlanet, HighEccentricity, NestedMoon, FastRotation/FastOrbit day→hour math, WideRings ring-width vs radius-multiplier + belt-ring exclusion, ShepherdMoon via `Owner.SystemBodies` parent lookup, LandableTerraformable/WithRings/HighGravity/LargeRadius, Bio/Geo signals, and the disabled path) with both firing and staying-silent cases. Caught two interesting gating facts while writing them: Bio/Geo signal toasts only fire for landable bodies (they sit after the `Landable == false` early return), and rotation/orbit periods are in days (×24 → hours). Smoke suite now 96 checks — ALL CHECKS PASSED.

Prior status (2026-08-09, REAL EliteJournalReader v3.7.4 VENDORED + PIPELINE RECONCILED):
...
- [x] Port Views/*.xaml from WPF to Avalonia — done: all views incl. the 5 Carto layouts, pop-out overlays, onboarding, message box, spansh selector. Interactive `MessageBoxWindow` dialog is wired to `MessageBoxRequester.Requested` in `App.axaml.cs`.
...
- [x] Reconcile the exo/prediction stubs with the real ODUtils engine — done (commit `ea50891`): the real exo engine was never committed — it sat untracked in `src/ODExplorer.Core/Exobiology/` (all 90 species with ODExplorer rules, variant matcher, `ExoData`, `ExoPlanetBuilder`, 27k-line `RegionMap`) while HEAD built the stubbed `ExoPredictionEngine`. Landed the real engine and deleted the BioScan stubs (`ExoGalaxyData`, `ExoGalaxyPosition`, `ExoPredictionData`, `ExoPredictionEngine`, `Exobiology`, `GalacticRegionsMap`, `OrganicStubs`); TestApp now runs the real engine and region/nebula rules. What *cannot* be reconciled: the remaining `Stubs/` files (`ODUtilsDatabaseBase`, `OdExplorerDatabaseDtos`, `ODUtilsSpanshStubs`, `ODUtilsModels`, `ODUtilsModelsBridge`, `WindowStateStub`, `ToastNotificationsStubs`, `ViewModelsWindowPositionStub`, `ModelsPopOutBaseStub`, `CountdownTimer`, journal/database store impls) mirror ODUtils types that are private/unrecoverable — they are vendored stand-ins, not fake logic (only `GalacticRegions` types had external callers, and those are now handled by the real `RegionMap`).
- [ ] Test on actual Linux desktop (GNOME/KDE/etc.) to verify notifications/audio work end-to-end — manual, requires a real desktop session. No Xvfb/headless runner in this environment; the in-repo verification is limited to the PulseAudio live-playback test (self-skips when no audio server) and the Avalonia build.
...
Latest handoff update (2026-08-11, headless UI test suite — #5)
- New NUnit test project `src/ODExplorer.UI.Avalonia.Tests/` (net8.0): `Avalonia.Headless` + `Avalonia.Headless.NUnit` 12.1.0, `Avalonia.Themes.Fluent` 12.1.0, NUnit 4.5.1, NUnit3TestAdapter 6.2.0, Microsoft.NET.Test.Sdk 17.13.0. Wired into ODExplorer.sln (Debug|Any CPU + Release|Any CPU mappings present for Core/UI/Tests — `dotnet sln add` dropped them; restored by hand).
- Run: `~/.dotnet/dotnet test src/ODExplorer.UI.Avalonia.Tests/ODExplorer.UI.Avalonia.Tests.csproj -c Release` => 10/10 passed, ~3s, stable.
- Files: `TestApp.axaml(.cs)` (headless Application, dark theme, ViewLocator template, wires `ODExplorer.Models.DispatcherHelper.Current` to the real `DispatcherAdapter` in OnFrameworkInitializationCompleted); `TestAppBuilder.cs` (`[assembly: AvaloniaTestApplication(...)]` + `UseHeadless(...).WithInterFont()`); `TestHarness.cs` (full MainViewModel graph over a per-invocation Guid-named temp SQLite DB, `OfflineEdsmApiService` override = no network); `ToastHostTests.cs` (5), `PopOutWindowTests.cs` (4), `ShellTests.cs` (1).
- **Root-cause found & fixed (the #5 regression)**: headless sessions without a real font use a fake font whose zero-advance glyphs cause an INFINITE CPU LOOP in `Avalonia.Base TextFormatterImpl.PerformTextWrapping`/`CreateEmptyTextLine` when a `TextWrapping="Wrap"` TextBlock gets its Text via a BINDING (LoadingView.StatusText). Fix: `.WithInterFont()` on the test AppBuilder (mirrors `Program.cs:22`). Literal/non-bound TextBlock text (ToastHost cards) never triggered it.
- Gotchas hit: (1) `SettingsStore.GetParams(popOut,count,commanderId)` is an "open" API that MUTATES the found entry (`Active=true`, SettingsStore.cs ~line 203) — read saved state via `GetCommanderPopOutParams(0)`; (2) stale-Resized race: `PopOutWindow.OnGeometryChanged` mutates shared `popOut.Position` from queued Resized events — reset test needs ~500ms drain after `Show()`; (3) namespace collision: inside `ODExplorer.UI.Avalonia.Tests`, bare `Avalonia.*` resolves to `ODExplorer.UI.Avalonia.*` — use `global::Avalonia.*`; (4) `Dispatcher.UIThread.RunJobs()` available in Avalonia 12.1.0 (used by the sync Shell test); (5) do NOT set `UseHeadlessDrawing=false` (breaks text layout: "Unable to locate 'Avalonia.Platform.IFontManagerImpl'"); (6) shared DB path caused "table 'CartoIgnoredSystems' already exists" under concurrent tests → per-test Guid DB; (7) `ToastMessage` ctor requires `(Title, Message)`.
- ToastHost card count read via `toastHost.Children.OfType<Canvas>().Single().Children.Count` (no test seam needed). `NotificationStore.ShowTestNotification()` works in tests.
- Verified: `dotnet build ODExplorer.sln -c Release` => 0 errors; TestApp smoke => 116 checks ALL CHECKS PASSED.
- Next: commit #5 (test project + sln wiring + this note) as a follow-up to `2edc881`, then reimplement the genus→species exo-prediction engine (real `ExoPlanet`/prediction pipeline in Core; hook FSSBodySignals + go-live re-prediction; feed `NotificationOptions` flags; validate against real journal folder).
